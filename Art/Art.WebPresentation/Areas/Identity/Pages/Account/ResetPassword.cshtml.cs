...
namespace Art.WebPresentation.Areas.Identity.Pages.Account
{
    ...
    public class ResetPasswordModel : PageModel
    {
        ...
        public ResetPasswordModel(IServiceProvider provider)
        {
            _provider = provider;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            var user = await _userEngine.UsersManager.FindByEmailAsync(Input.Email);
            if (user == null)
            {
            ...
            var result = await _userEngine.UsersManager.ResetPasswordAsync(user, Input.Code, Input.Password);
            if (result.Succeeded)
            {
                return RedirectToPage("./ResetPasswordConfirmation");
            }
